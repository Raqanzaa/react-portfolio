import React, { useState } from "react";

const Navbar = () => {
  const [activeSection, setActiveSection] = useState("home");

  const handleNavClick = (section) => {
    setActiveSection(section);
  };

  // Fungsi untuk menentukan kelas aktif
  const getNavLinkClass = (section) => {
    return `nav-link block py-2 px-3 rounded hover:bg-gray-100 md:hover:bg-transparent md:p-0 md:dark:hover:bg-transparent md:dark:hover:text-blue-700 dark:border-gray-700 ${
      activeSection === section
        ? "text-blue-700 dark:text-primary md:dark:text-primary"
        : "text-gray-900 dark:text-white md:hover:text-blue-700 dark:hover:text-white"
    }`;
  };

  return (
    <nav className="bg-white dark:bg-gray-900/80 fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a
          href="https://flowbite.com/"
          className="flex items-center space-x-3 rtl:space-x-reverse"
        >
          <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">
            Raqanzaa.
          </span>
        </a>
        <button
          data-collapse-toggle="navbar-sticky"
          type="button"
          className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
          aria-controls="navbar-sticky"
          aria-expanded="false"
        >
          <span className="sr-only">Open main menu</span>
          <svg
            className="w-5 h-5"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 17 14"
          >
            <path
              stroke="currentColor"
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth="2"
              d="M1 1h15M1 7h15M1 13h15"
            />
          </svg>
        </button>
        <div
          className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1"
          id="navbar-sticky"
        >
          <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-transparent md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0">
            <li>
              <a
                href="#home"
                className={getNavLinkClass("home")}
                data-section="home"
                onClick={() => handleNavClick("home")}
              >
                Home
              </a>
            </li>
            <li>
              <a
                href="#about"
                className={getNavLinkClass("about")}
                data-section="about"
                onClick={() => handleNavClick("about")}
              >
                About
              </a>
            </li>
            <li>
              <a
                href="#project"
                className={getNavLinkClass("project")}
                data-section="project"
                onClick={() => handleNavClick("project")}
              >
                Project
              </a>
            </li>
            <li>
              <a
                href="#journey"
                className={getNavLinkClass("journey")}
                data-section="journey"
                onClick={() => handleNavClick("journey")}
              >
                Journey
              </a>
            </li>
            <li>
              <a
                href="#skills"
                className={getNavLinkClass("skills")}
                data-section="skills"
                onClick={() => handleNavClick("skills")}
              >
                Skills
              </a>
            </li>
            <li>
              <a
                href="#contact"
                className={getNavLinkClass("contact")}
                data-section="contact"
                onClick={() => handleNavClick("contact")}
              >
                Contact
              </a>
            </li>
          </ul>
        </div>
      </div>
    </nav>
  );
};

export default Navbar;